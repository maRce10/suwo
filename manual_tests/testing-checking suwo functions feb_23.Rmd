---
title: "testing-checking suwo functions"
...
pkgs <- c("remotes", "devtools", "pbapply", "RCurl", "rjson", "kableExtra", "knitr", "formatR", "rprojroot", "xaringanExtra" ,"microbenchmark", "pkgdown", "maRce10/suwo", "covr", "pkgcheck", "visNetwork", "codemetar", "ropensci-review-tools/pkgstats", "ropensci/tic", "test_that")
...
# run internals
run_suwo_int <- function(){ 
out <- lapply(list.files(pattern = "suwo_int", path = "./R", full.names = T), source)}
...
write.sound.files <- function(path = tempdir(), extensible = T, other.formats = FALSE, simulated = FALSE){
  # setwd(path)
data(lbh1, package = "suwo")
data(lbh2, package = "suwo")
...
unlink(list.files(pattern = "suwo\\.pdf", path = "."))
...
mydir <- file.path (tempdir (), "suwo")
gert::git_clone ("https://github.com/maRce10/suwo.git", path = mydir)
