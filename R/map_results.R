...
#'
#' \code{map_results} creates maps to visualize the geographic spread of 'Xeno-Canto'
#'   recordings.
#' @usage map_results(X, img = TRUE, it = "jpeg", res = 100, labels = FALSE,
...
#' map_results(X, img = FALSE)
#'
#' #create leaflet map
#' map_results(X, leaflet.map = TRUE)
#' }
#'
#' @references {
...
map_results <- function(X, img = TRUE, it = "jpeg", res = 100, labels = FALSE,
                   path = NULL, leaflet.map = FALSE,
                   leaflet.cluster = FALSE) {
  ...
  argms <- methods::formalArgs(map_results)
    ...
    for(i in sort(unique(X$species))) {
      ...
      {if (abs(max(y$Longitude) - min(y$Longitude)) < 38) buf <- 12 else buf <- 5
       ...
       if (img){
         ...
         #add empty  map
         maps::map("world", xlim = c(min(y$Longitude) - buf, max(y$Longitude) + buf), interior = FALSE,
         ...
         #add map
         maps::map("world", xlim = c(min(y$Longitude) - buf, max(y$Longitude) + buf), add = TRUE,
      ...
      # add markers
      if (leaflet.cluster)
      leaf.map <- leaflet::addAwesomeMarkers(map = leaf.map, ~Longitude, ~Latitude, icon=icons, label=~labels, popup = content, data = X, clusterOptions = leaflet::markerClusterOptions(),
                                             clusterId = "rec.cluster") else
...
##############################################################################################################
#' alternative name for \code{\link{map_results}}
#'
#' @keywords internal
#' @details see \code{\link{map_results}} for documentation. \code{\link{xcmaps}} will be deprecated in future versions.
#' @export
