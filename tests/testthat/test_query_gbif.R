...
test_that("test verbose FALSE", {
  ...
  msg <- capture.output(a <- query_gbif(species = 'Glaucis dohrnii',
                                        format =  "image", verbose = FALSE,
                                        pb = FALSE))
...
test_that("test all_data FALSE", {
  ...
  expect_true(all(expected_col_names %in% query_col_names) && all(query_col_names %in% expected_col_names),
