...
test_that("search Glaucis dohrnii photos specific data set", {
  ...
  df1 <- query_gbif(species = 'Glaucis dohrnii', format =  "image", dataset = "b1047888-ae52-4179-9dd5-5448ea342a24")
...
test_that("search Aristolochia baetica photos in parallel", {
  ...
  df1 <- query_gbif(species = 'Aristolochia baetica', format = "image", cores = cores)
...
test_that("test verbose FALSE", {
  ...
  msg <- capture.output(a <- query_gbif(species = 'Glaucis dohrnii', format =  "image", verbose = FALSE, pb = FALSE))
...
test_that("test all_data FALSE", {
  ...
  df1 <- query_gbif(species = 'Glaucis dohrnii', format =  "image", all_data = FALSE)
