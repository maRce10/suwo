...
Finding data using [suwo](https://github.com/maRce10/suwo) follows a basic sequence. The following diagram illustrates this workflow and the main functions involved:
...
1. If multiple repositories are queried, the resulting metadata data frames can be merged into a single data frame using the [merge_metadata()](https://marce10.github.io/suwo/reference/merge_metadata.html) function. 

1. Check for duplicate records in their datasets using the [find_duplicates()](https://marce10.github.io/suwo/reference/find_duplicates.html) function. Candidate duplicated entries are identified based on matching species name, country, date, user name, and geographic coordinates. User can double check the candidate duplicates and decide which records to keep, which can be done with [remove_duplicates()](https://marce10.github.io/suwo/reference/remove_duplicates.html).
...
The following table summarizes the available [suwo](https://github.com/maRce10/suwo) query functions and the types of metadata they retrieve:
...
This is an example code of a query of _Turdus rufiventris_ (a bird species) sounds from iNaturalist and Wikiaves (we print the first 4 rows of each output data frame):
...
truf_inat <- query_inaturalist(term = "Turdus rufiventris")
...
The code above examplifies the most common use of query functions, which applies also to the function [query_gbif()](https://marce10.github.io/suwo/reference/query_gbif.html). The following sections provide more details on the two query functions that require special considerations.    

## Query Macaulay Library interactively

[query_macaulay()](https://marce10.github.io/suwo/reference/query_macaulay.html) is the only interactive function. This means that when users run a query the function opens a browser window to the [Macaulay Library's search page](https://search.macaulaylibrary.org/catalog), where the users must download a .csv file with the metadata:
...
## API key to query Xeno-Canto

[Xeno-Canto](https://www.xeno-canto.org/) requires users to obtain a free API key to use their API v3. Users can get their API key by creating an account at [Xeno-Canto's registering page](https://xeno-canto.org/auth/register). Once users have their API key, they can use it in the [query_xenocanto()](https://marce10.github.io/suwo/reference/query_xenocanto.html) function by providing it as the `key` argument. Here is an example of a query for _Turdus rufiventris_ sounds:

```{r, eval = FALSE}

# Query Xeno-canto for Phaethornis anthophilus sounds
# replace "YOUR_XC_API_KEY" with your key
tur_ruf_xc <- query_xenocanto(term = "Turdus rufiventris", key = "YOUR_XC_API_KEY")
...
head(tur_ruf_sound_list$xenocanto[, suwo:::.format_query_output(only_basic_columns = TRUE)], 4)
...
To show case the function, we will use the metadata from the previous example of _Turdus rufiventris_ sounds from iNaturalist. Let's assume that the initial query was done a while ago and we want to update it to include any new records that might have been added since then. The following code removes all observations recorded after 2024-12-31 to simulate an old query:


```{r}
# exclude new observations (simulate old data)
old_truf_inat <- subset_metadata(truf_inat, subset = date <= "2024-12-31" | is.na(date))

# update "old" data
updated_truf_inat <- update_metadata(metadata = old_truf_inat)

# compare number of records
nrow(truf_inat) == nrow(updated_truf_inat)

```

Note the use of the [suwo](https://github.com/maRce10/suwo) function [subset_metadata()](https://marce10.github.io/suwo/reference/subset_metadata.html) to filter the data frame by date. This is a modified version of `base::subset()` that makes sure that the attributes of the data frame are preserved, which are needed by subsequent [suwo](https://github.com/maRce10/suwo) functions working on metadata. Those attributes will be lost if you use `dplyr::filter()`, `base::subset()` or the extract operator `[` (e.g. `metadata[1:3, ]`).
...
truf_inat <- query_inaturalist(term = "Turdus rufiventris", format = "sound")
...
# merge metadata
merged_metadata <- merge_metadata(truf_inat, truf_gbf, truf_ml)
...
Note that in such a multi-repository query, all query functions use the same search term (i.e. species name) and media format (e.g., sound, image, video). To facilitate this, users can set the global options `term` and `format` so they do not need to specify them in each query function:

```{r, eval = FALSE}

options(term = "Turdus rufiventris", format = "sound")
truf_inat <- query_inaturalist()
truf_gbf <- query_gbif()
...
azam_gbf <- query_gbif(term = "Amanita zambiana", format = "image")
...
azam_files <- download_media(metadata = azam_gbf, path = out_folder)
...
for (i in 1:6) {
  img <- jpeg::readJPEG(file.path(out_folder, azam_files$download_file_name[i]))
  plot(
    1:2,
    type = 'n',
    axes = FALSE
...
# create folder for images
out_folder <- file.path(tempdir(), "amanita_zambiana2")
dir.create(out_folder)

# download media files creating sub-directories by country
azam_files <- download_media(metadata = azam_gbf,
                             path = out_folder,
                             folder_by = "country")
