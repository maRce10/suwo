...
options(all_data = FALSE, raw_data = FALSE)
...
comb_dat <- rbind(xc_adf, gb_adf)

# str(comb_dat)
# 
# View(comb_dat)

# Method 1: Using compare.linkage for detailed comparison
pairs <- compare.dedup(
  comb_dat, 
  blockfld = list("country", "format"),  # Block by country and species to reduce comparisons
  # strcmp = c("user_name"), # Use string comparison for these fields
  strcmpfun = jarowinkler  # Use Jaro-Winkler similarity
)


table(pairs$pairs$is_match)

pairs$frequency

mtch <- pairs$pairs[!is.na(pairs$pairs$is_match), ]
