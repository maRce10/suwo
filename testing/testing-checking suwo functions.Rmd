...
pkgs <- c("remotes", "curl", "RCurl", "jsonlite", "crayon", "devtools", "leaflet", "maps", "pkgcheck", "pkgstats", "testthat", "styler", "cli", "beepr", "RecordLinkage")
...
metadata2 <- merge_metadata(xc_adf, gb_adf,ml_adf)
dup_metadata <- find_duplicates(metadata = metadata2, sort = FALSE)

View(dup_metadata)
all(table(dup_metadata$duplicate_group) > 1)
length(unique(dup_metadata$duplicate_group))

View(dup_metadata[!is.na(dup_metadata$duplicate_index),])

View(dup_metadata[dup_metadata$date == "1979-09-29" & !is.na(dup_metadata$date),])
View(metadata[metadata$date == "1979-09-29" & !is.na(metadata$date),])

metadata <- readRDS(file = "./testing/find_duplicates_test_data.rds")
dup_metadata <- find_duplicates(metadata = metadata)
...
usethis::use_data(ml_taxon_code, internal = TRUE, overwrite = TRUE)
...
# find issues in metadata formatting
```{r}
...
# 
# saveRDS(adt_list, "./testing/adt_list.rds")

adt_list <- readRDS("./testing/adt_list.rds")
...
usethis::use_data(tur_ruf_list, internal = FALSE, overwrite = TRUE)

adf_list <- lapply(adt_list, function(x) x[, basic_colms])

comb_adf <- do.call(rbind, adf_list)

(is_num <- sapply(comb_adf, is.numeric))

nmax <- 600
lapply(comb_adf[ , !is_num], function(x) {
  y <- unique(x)
  if (length(y) > nmax) y <- sample(y, nmax)
  return(y)
  })


comb_adf[grep("Plurinational", comb_adf$country), ]

adt_list$gb_adt_i[is.na(adt_list$gb_adt_i$file_extension), ]


View(comb_adf[is.na(comb_adf$file_extension), ])
unique(comb_adf$file_extension)

a[a$country == "Bolivia (Plurinational State of)" & !is.na(a$country), ]

table(a$repository)

a[is.na(a$file_extension),]

gb_adt <- query_gbif(term = term, format = "sound", all_data = TRUE)

W <- gb_adt[grep("museum",gb_adt$file_extension), ]


gb_adt$file_extension

gb_adt[, c("media_format", "file_extension")]

table(gb_adt$media_format, gb_adt$file_extension)

gb_adf$occurrenceid[is.na(gb_adf$file_extension)]
```
